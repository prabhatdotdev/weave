...
func TestSubscribeFailsDuringRecovery(t *testing.T) {
	...
	err = broker.Subscribe(context.Background(), "test-topic", func(context.Context, *core.Message) error { return nil })
	if err == nil {
...
func TestCallFailsDuringRecovery(t *testing.T) {
	...
	// Call should fail
	msg := core.NewTextMessage("test")
	_, err = broker.Call(context.Background(), "test-topic", msg)
	if err == nil {
